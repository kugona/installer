...
TRANSLATIONS_EN[node_enter_custom_core_url]="Enter custom core URL (leave empty to skip): "
...
TRANSLATIONS_RU[node_enter_custom_core_url]="Введите URL custom core (оставьте пустым, чтобы не добавлять): "
...
create_node_docker_compose() {
    local certificate="$1"
    local custom_core_url="$2"
    local custom_core_url_env=""

    if [ -n "$custom_core_url" ]; then
        custom_core_url_env="            - CUSTOM_CORE_URL=$custom_core_url"
    fi
...
setup_node() {
    ...
    local custom_core_url
    custom_core_url=$(prompt_custom_core_url)

    # Create node docker-compose
    create_node_docker_compose "$CERTIFICATE" "$custom_core_url"
